...
mod tests {
    ...
    fn test_process_origins_and_build_subgraph() {
        ...
        let (visited, unknown_origins) = swhids_subgrapher::process_origins_and_build_subgraph(
            &graph,
            origins.into_iter(),
            true,
        );
        ...
        let (empty_nodes, empty_unknown) =
            swhids_subgrapher::process_origins_and_build_subgraph(&graph, iter::empty(), false);
        ...
        let (invalid_nodes, invalid_unknown) =
            swhids_subgrapher::process_origins_and_build_subgraph(
                &graph,
